...
class DRQNModel(nn.Module):
    def __init__(self):
        ...
        self.layer3 = nn.Conv2d(64, 64, kernel_size=3, stride=1, padding=1, bias=True)
        self.pool3 = nn.MaxPool2d(kernel_size=2, stride=2, ceil_mode=True)
        self.recurrent_layer = nn.GRU(input_size=256, hidden_size=256, num_layers=1)
        self.fc_layer1 = nn.Linear(256, 128)
        self.fc_layer2 = nn.Linear(128, 3)
    ...
    def forward(self, input_tensor):
        ...
        out3 = F.relu(self.layer3(pooled2))
        pooled3 = self.pool3(out3)
        flattened = pooled3.view(-1, 256).unsqueeze(1)
        rnn_output, hidden_state = self.recurrent_layer(flattened)
        fc1_output = self.fc_layer1(rnn_output)
        fc2_output = self.fc_layer2(fc1_output).squeeze(1)
        return fc2_output

    def save(self, file_name='model.pth', folder_path='./model_drqn'):
        os.makedirs(folder_path, exist_ok=True)
        full_path = os.path.join(folder_path, file_name)
        torch.save(self.state_dict(), full_path)